...
inline std::string cl_pns3d =
"#define WG 256\n"
"#pragma OPENCL EXTENSION cl_khr_fp64 : enable\n"
"double get_A1(double eta, double3 v, double r_dot, double m, double r){\n"
"    return -(1 + 3*eta) * dot(v, v) + 1.5 * eta * r_dot * r_dot + 2*(2 + eta) * m / r;\n"
"}\n"
"double get_B1(double eta){\n"
"    return 2*(2 - eta);\n"
"}\n"
"double get_A2(double eta, double3 v, double r_dot, double m, double r){\n"
"    double retval = - eta * (3 - 4 * eta) * pow(dot(v, v),2);\n"
"    retval += 0.5 * eta * (13 - 4*eta) * dot(v, v) * m / r;\n"
"    retval += 1.5 * eta * (3 - 4*eta) * dot(v, v) * r_dot * r_dot;\n"
"    retval += (2 + 25*eta + 2 *eta *eta) * r_dot * r_dot * m /r;\n"
"    retval += - 1.875 * eta * (1 - 3*eta) * pow(r_dot,4) - 0.75 * (12 + 29 * eta) * (m /r) * (m/r);\n"
"    return retval;\n"
"}\n"
"double get_B2(double eta, double3 v, double r_dot, double m, double r){\n"
"    double retval = 0.5 * eta * (15 + 4 *eta) * dot(v, v);\n"
"    retval += -1.5 * eta * (3+ 2*eta) * r_dot *r_dot;\n"
"    retval += -0.5 * (4 + 41*eta + 8*eta*eta) * m /r;\n"
"    return retval;\n"
"}\n"
...
"    double3 pi=vload3(0,positions+3*i);\n"
"    double this_mass=masses[i];\n"
"    double3 this_vel=vload3(0,velocities+3*i);\n"
...
"        uint j=tile+lid;\n"
"        if(j<N){\n"
"            spos[lid]=vload3(0,positions+3*j);\n"
"            sm[lid]=masses[j];\n"
"            svel[lid]=vload3(0,velocities+3*i);\n"
"        }else{\n"
...
"            acc-=(0.8*eta*(m*m)/(r*r*r)*(n*r_dot*half_a-v*half_b))/(C*C*C*C*C*sm[k])*G;\n"
"        }\n"
"        barrier(CLK_LOCAL_MEM_FENCE);\n"
"    }\n"
"    vstore3(acc,0,acc_out+3*i);\n"
"}\n"
"__kernel void pn2(\n"
"    const uint N,\n"
"    const double G,\n"
...
"            spos[lid]=vload3(0,positions+3*j);\n"
"            sm[lid]=masses[j];\n"
"            svel[lid]=vload3(0,velocities+3*j);\n"
"        }else{\n"
"            spos[lid]=(double3)(0.0,0.0,0.0);\n"
"            svel[lid]=(double3)(0.0,0.0,0.0);\n"
"            sm[lid]=0.0;\n"
...
"            double3 n=x/r;\n"
"            double r_dot=dot(v,n);\n"
"            double a_components=-(1+3*eta)*dot(v,v)+1.5*eta*r_dot*r_dot+2*(2+eta)*m/r;\n"
...
inline std::string cl_pns3d_lite = 
"#define WG 256\n"
"float get_A1(float eta, float3 v, float r_dot, float m, float r){\n"
"    return -(1 + 3*eta) * dot(v, v) + 3/2 * eta * r_dot * r_dot + 2*(2 + eta) * m / r; \n"
"}\n"
"\n"
"float get_B1(float eta){\n"
"    // 2(2 - η)\n"
"    return 2*(2 - eta); \n"
"}\n"
"\n"
"float get_A2(float eta, float3 v, float r_dot, float m, float r){\n"
"    float retval = - eta * (3 - 4 * eta) * dot(v, v) * dot(v,v);\n"
...
"}\n"
"\n"
"float get_B2(float eta, float3 v, float r_dot, float m, float r){\n"
"    float retval = 1/2 * eta * (15 + 4 *eta) * dot(v, v);\n"
...
"}\n"
"\n"
"float get_B25(float eta, float3 v, float r_dot, float m, float r){\n"
"    return dot(v, v) + 3 * m /r;\n"
"}\n"
"\n"
"__kernel void pn25(\n"
...
"    float this_mass = masses[i];\n"
"    float3 this_vel = velocities[i];\n"
"\n"
"    __local float3 spos[WG];\n"
"    __local float  sm  [WG];\n"
"    __local float3  svel[WG];\n"
"\n"
"    for (uint tile = 0; tile < N; tile += WG) {\n"
"        uint lid = get_local_id(0);\n"
"        uint j   = tile + lid;\n"
...
"            uint global_j = tile + k;\n"
"            if (global_j >= N || global_j == i) continue;\n"
"            float3 x = (spos[k] - pi + 1e-11);\n"
"            float r = sqrt(dot(x,x)) + 1e-11;\n"
"            float m = sm[k] + this_mass;\n"
"            float eta = sm[k] * this_mass / (m*m);\n"
"            \n"
"            float3 v = this_vel - svel[k];\n"
"            float3 n = x / r;\n"
"            float r_dot = dot(v, n);\n"
"\n"
"            float a_components = get_A1(eta, v, r_dot, m, r) + get_A2(eta, v, r_dot, m, r);\n"
"            float b_components = get_B1(eta                ) + get_B2(eta, v, r_dot, m, r);\n"
"\n"
"            float half_a = get_A25(eta, v, r_dot, m, r);\n"
"            float half_b = get_B25(eta, v, r_dot, m, r);\n"
"\n"
"            acc += n * sm[k] / (r*r) * G; \n"
"            acc += -((n * a_components + v *r_dot * b_components) * m / (r*r)) / (C*C*C) * G / sm[k]; \n"
...
"            float r_dot = dot(v,n);"
"        \n"
"            float a_components = -(1 + 3*eta)*dot(v,v) + 3/2 * eta * r_dot * r_dot + 2*(2 + eta) * m / r;\n"
"            float b_components = get_B1(eta                );\n"
"        \n"
"        \n"
"            acc += n * sm[k] / (r*r) * G; \n"
"            acc += -((n * a_components + v *r_dot * b_components) * m / (r*r)) / (C*C*C*this_mass) * G;// \n"
