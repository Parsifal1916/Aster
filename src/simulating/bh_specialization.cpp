...
template <>
size_t BHT<vec2>::subdivide(int n){
    size_t child_index = nodes.size();
    Node<vec2>& cnode = nodes[n]; 
...
template <>
size_t BHT<vec3>::subdivide(int n){
    size_t child_index = nodes.size();
    Node<vec3>& cnode = nodes[n]; 
...
*/
template <>
int BHT<vec2>::opt_position(vec2 p, vec2 center){ 
    bool 
        x = p.x > center.x,
        y = p.y > center.y
    ;
...
*/
template <>
int BHT<vec3>::opt_position(vec3 p, vec3 center){ 
    bool 
        x = p.x > center.x,
        y = p.y > center.y,
        z = p.z > center.z
